/*  =========================================================================
    zproject_pg_selftest.c - run selftests
...
// Tests for draft public classes:
    { "testclass", testclass_test, false, true, NULL },
...
{
    test_item_t *item;
    printf ("Running zproject_pg selftests...\n");
    for (item = all_tests; item->testname; item++) {
        if (streq (item->testname, "private_classes"))
            continue;
        if (!item->subtest)
            item->test (verbose);
#ifdef ZPROJECT_PG_BUILD_DRAFT_API // selftest is still in draft
        else
            zproject_pg_private_selftest (verbose, item->subtest);
#endif // ZPROJECT_PG_BUILD_DRAFT_API
    }
...
main (int argc, char **argv)
{
    ...
    for (argn = 1; argn < argc; argn++) {
        ...
        ||  streq (argv [argn], "-h")) {
            puts ("zproject_pg_selftest.c [options] ...");
    ...
    if (test) {
        printf ("Running zproject_pg test '%s'...\n", test->testname);
        if (!test->subtest)
            test->test (verbose);
#ifdef ZPROJECT_PG_BUILD_DRAFT_API // selftest is still in draft
        else
            zproject_pg_private_selftest (verbose, test->subtest);
#endif // ZPROJECT_PG_BUILD_DRAFT_API
    }
    else
        test_runall (verbose);
